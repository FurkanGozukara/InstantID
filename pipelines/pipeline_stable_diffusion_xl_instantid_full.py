...
class StableDiffusionXLInstantIDPipeline(StableDiffusionXLControlNetPipeline):
    ...
    def enable_sdpa(self):
        if hasattr(torch.nn.functional, 'scaled_dot_product_attention'):
            def sd_forward(self, query, key, value):
                return torch.nn.functional.scaled_dot_product_attention(query, key, value, dropout_p=0.0, is_causal=False)

            def fn_recursive_set_sdpa(module):
                if hasattr(module, "set_use_memory_efficient_attention_xformers"):
                    module.set_use_memory_efficient_attention_xformers(False, sd_forward)
                for child in module.children():
                    fn_recursive_set_sdpa(child)

            fn_recursive_set_sdpa(self.unet)
            fn_recursive_set_sdpa(self.text_encoder)
            if hasattr(self, 'text_encoder_2') and self.text_encoder_2 is not None:
                fn_recursive_set_sdpa(self.text_encoder_2)
            fn_recursive_set_sdpa(self.vae)
            print("SDPA enabled successfully")
        else:
            print("SDPA not available in this PyTorch version. Please use PyTorch 2.0 or later.")
    ...
    def cuda(self, dtype=torch.float16, use_xformers=False):
            ...
            if use_xformers:
                try:
                    ...
                    logger.warn("xformers is not available. Continuing without it.")
            return self
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if isinstance(controlnet, ControlNetModel):
            ...
            raise ValueError(f"Unsupported controlnet type: {type(controlnet)}. Expected ControlNetModel or MultiControlNetModel.")
        ...
        # 5. Prepare timesteps
        self.scheduler.set_timesteps(num_inference_steps, device=_device)
        ...
        if self.unet.config.time_cond_proj_dim is not None:
            ...
            ).to(device=_device, dtype=latents.dtype)
